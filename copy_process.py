...
# Fetch a single flight path image by disc ID
def fetch_flight_data_image(disc_id):
    conn = connect_db()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT id, flight_data_image FROM disc_flight_data WHERE id = %s", (disc_id,))
...
def detect_line_color(image):
    ...
    # Check each color range for significant pixels in the image
    max_pixels = 0
    detected_color = None
...
# Process the flight path from a black and white image (used for red and blue lines)
def process_flight_path_from_image(image_data):
    # Convert the binary image data to a format OpenCV can handle
...
# Process the flight path for a specific disc ID
def process_single_flight_path(disc_id):
    disc = fetch_flight_data_image(disc_id)
    if disc:
        try:
            ...
            if detected_color in ['red', 'blue']:  # Use contrast for red and blue lines
                flight_path_coords = process_flight_path_from_image(image_data)
            else:  # Use HSV for green and orange lines
...
if __name__ == "__main__":
    # Set the disc ID to process a single image
    disc_id = 1  # Replace with the actual disc ID you want to test
    process_single_flight_path(disc_id)
